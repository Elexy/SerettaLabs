...
static void homePage(BufferFiller& buf) {
    ...
    for (byte i = 0; i < NUM_MESSAGES; ++i) {
        ...
        if (history_len[j] > 0) {
            ...
            {
              ((byte*) &dataRx)[k] = history_rcvd[j][k+1];
              //sprintf(strDataResult, " %x ", history_rcvd[j][k]);
              //buf.emit_p(PSTR(" $F"), strDataResult);
              
            }
            buf.emit_p(PSTR(" Pump $D, Temp $D e-1 C, Flow $D L/h"), 
                       dataRx.pump, dataRx.tempOut, dataRx.flow);
        }
    }
    long t = millis() / 1000;
    word h = t / 3600;
