...
//    if ( payloadData.temp < 19.0 && payloadData.temp < 20) {
//      centralData.heat = 1;
//      while ( !rf12_canSend())
//        rf12_recvDone();
//      char sending = rf12_sendStart(0, ptr, len);
//      rf12_sendStart(30, centralData, sizeof centralData);
//      rf12_sendWait(2);
//    }
//
////    char sending = rf12_sendStart//rf12_easySend(ptr, len);
...
//        sending = rf12_sendStart(0, ptr, len); //rf12_easySend(0, 0); // always returns 1
//    if (sending) {
...
static void newReadings() {
    ...
    payloadData.moved = motion != prevMotion;    
    prevMotion = motion;

    if (reportTimer.poll(30000) || payloadData.moved) {
        payloadData.light = light;
#if SHT1X
        float humi, temp;
        ...
        if (humi > 1) {
            payloadData.humi = humi + 0.5;
            payloadData.temp = 10 * temp + 0.5;
        }
#else
        payloadData.humi = 0;
        payloadData.temp = readout1wire();
#endif
        payloadData.lobat = rf12_lowbat();
    }
}
...
void loop() {
    if (periodicSleep(1000)) {
        ...
        Serial.print((int) payloadData.light);
        Serial.print(' ');
        Serial.print((int) payloadData.moved);
        Serial.print(' ');
        Serial.print((int) payloadData.humi);
        Serial.print(' ');
        Serial.print((int) payloadData.temp);
        Serial.print(' ');
        Serial.print((int) payloadData.lobat);
        Serial.println();
        delay(3); // make sure tx buf is empty before going back to sleep

        //wakeupToSend(&payloadData, sizeof payloadData);
        if ( payloadData.temp < 19.0 && payloadData.temp < 20) {
          centralData.heat = 1;
          while ( !rf12_canSend())
            rf12_recvDone();
          char sending = rf12_sendStart(0, payloadData, sizeof payloadData);
          rf12_sendStart(30, centralData, sizeof centralData);
          rf12_sendWait(2);
        }
