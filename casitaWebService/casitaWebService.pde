...
static void jsonPage(const char* data, BufferFiller& buf) {
    ...
    buf.emit_p(PSTR("$F\r\n[$D,$D,$D,$D,$D,$D,$D,$D,$D,$D,$D,$D,$D,$D,$D,$D]"),
      okHeader,
      room.heat, room.temp, room.auxHeat,room.panelOut,
      panel.tempOut, panel.tempIn, panel.tempAmb,
      casita.tankIn, casita.tankBottom, casita.tankTop, casita.xchangeIn, 
        casita.xchangeOut, casita.afterHeater, casita.floorIn, casita.floorFlow, casita.errorCode);
}
...
void loop(){
    ...
    if (pos) {
        ...
        if (strncmp("GET / ", data, 6) == 0)
            ...
            sendPage(data, bfill);
        else if (strncmp("GET /j", data, 6) == 0)
            jsonPage(data, bfill);        
        else
            bfill.emit_p(PSTR(
